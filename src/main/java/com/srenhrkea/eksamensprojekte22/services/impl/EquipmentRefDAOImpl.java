...
import com.srenhrkea.eksamensprojekte22.services.EquipmentRefDAO;
...
public class EquipmentRefDAOImpl implements EquipmentRefDAO {
  ...
  @Override
  public boolean delete(EquipmentRef equipmentRef) {

    String sql = "DELETE FROM equipmentref WHERE idEquipmentRef=?;";
    try (PreparedStatement psts = conn.prepareStatement(sql)) {

      psts.setInt(1, equipmentRef.getIdEquipmentRef());

      int executeUpdate = psts.executeUpdate();

      if (executeUpdate == 1) {
        System.out.println("Equipment Reference with ID " + equipmentRef.getIdEquipmentRef() + " is deleted.::");
        return true;
      }
    } catch (Exception e) {
      e.printStackTrace();
